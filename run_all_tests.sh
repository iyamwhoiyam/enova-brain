...
run () {   # run <label> <cmd...>  — a step passes if it exits 0 AND prints no "FAIL"
  local label="$1"; shift
  # Skip gracefully if the test script is absent (some fixtures live only in the GitHub
  # repo / cloud workspace and were never saved to local disk — see WHATS_NEW.txt). This
  # keeps the suite runnable on a partial checkout instead of hard-failing on a missing file.
  local a; for a in "$@"; do
    case "$a" in
      *.js|*.sh) if [ ! -f "$a" ]; then
        printf "  ⏭  %-26s (missing %s — recover from repo)\n" "$label" "$a"
        skip=$((skip+1)); skipped_list="$skipped_list $label"; return; fi ;;
    esac
  done
...
echo "───────────────────────────────────────────────────────────────────"
total=$((pass+fail))
[ $skip -gt 0 ] && echo "  ⏭  $skip skipped (test file absent on this checkout):$skipped_list"
if [ $fail -eq 0 ]; then
  echo "  ALL $total PRESENT CHECKS PASSED — safe to deploy.$([ $skip -gt 0 ] && echo " ($skip skipped — recover those from the GitHub repo)")"
  exit 0
else
